...
export const SearchHighlightConfig = Facet.define<
	SearchHighlightOptions,
	Required<SearchHighlightOptions>
>({
...
export function highlightSearchMatches(
	...
	if (options) {
		ext.push(
			SearchCompartment.of(SearchHighlightConfig.of(cloneDeep(options)))
		);
	}
	return ext;
}
...
export function reconfigureSearchHighlighter(options: SearchHighlightOptions) {
	return SearchCompartment.reconfigure(
		SearchHighlightConfig.of(cloneDeep(options))
	);
}
...
const searchHighlighter = ViewPlugin.fromClass(
	class {
		...
		update(update: ViewUpdate) {
			let reconfigured =
				JSON.stringify(
					update.startState.facet(SearchHighlightConfig).highlightQuery
				) !==
				JSON.stringify(
					update.state.facet(SearchHighlightConfig).highlightQuery
				);
		...
		updateDebouncer(view: EditorView) {
			this.highlightDelay = view.state.facet(
